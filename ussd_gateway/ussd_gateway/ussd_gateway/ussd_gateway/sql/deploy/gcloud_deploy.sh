...
gcloud run deploy $SERVICE_NAME \
  ...
  --allow-unauthenticated \
  --set-env-vars="DB_INSTANCE=$DB_INSTANCE_NAME,DB_NAME=$DB_NAME,DB_USER=$DB_USER,DB_PASS=$DB_PASS" \
  --add-cloudsql-instances=$PROJECT_ID:$REGION:$DB_INSTANCE_NAM#!/bin/bash

# === CONFIGURATION ===
<<<<<<< HEAD:ussd_gateway/ussd_gateway/ussd_gateway/ussd_gateway/sql/deploy/gcloud_deploy.sh
PROJECT_ID="inehealth-mvp-462020"   # ✅ Replace if needed
=======
PROJECT_ID="inehealth-mvp-462020"
>>>>>>> 42e32eb (Add Cloud Build config and sync with remote):deploy/gcloud_deploy.sh
REGION="us-central1"
SERVICE_NAME="ussd-gateway"
DB_INSTANCE_NAME="health-db"
DB_NAME="ninehealth"
